...
### 아파치 웹 서버 modsecurity 실습 결과를 로그로 저장 한다.
...
| 항목        | 변화량             | 간단한 원인 요약                             |
| --------- | --------------- | ------------------------------------- |
| `%usr`    | 17 → 28 (%)     | CPU 사용자 영역 사용 증가, 다수의 curl 실행으로 인한 부담 |
| `%sys`    | 18 → 54 (%)     | 네트워크 스택 처리 등 커널 작업 급증                 |
| `%idle`   | 37 → 0 (%)      | CPU가 쉬지 않고 모든 자원을 사용                  |
| `LOC/s`   | 786.00 → 656.00 | 인터럽트 기반에서 polling 처리 방식으로 전환 가능성      |
| `SCHED/s` | 210.00 → 26.00  | 많은 프로세스가 block 상태로 머물러 스케줄러 호출 감소     |
